...
describe("StudySessionService", () => {
  ...
  describe("start", () => {
    ...
    it("should start a session with courseId successfully", async () => {
      const course = makeCourse({ userId: userAuthUser.id });
      const session = makeStudySession({ userId: userAuthUser.id, courseId: course.id, status: StudySessionStatus.IN_PROGRESS, minutes: 0 });
      ...
      studySessionRepositoryMock.findActiveByUser.mockResolvedValue([]);
      ...
      expect(studySessionRepositoryMock.findActiveByUser).toHaveBeenCalledWith(userAuthUser.id);
    ...
    it("should start a session with moduleId successfully", async () => {
    ...
    it("should start a session with disciplineId successfully", async () => {
    ...
    it("should throw ValidationError when no scope is provided", async () => {
      ...
      expect(studySessionRepositoryMock.findActiveByUser).not.toHaveBeenCalled();
    ...
    it("should throw ValidationError when user already has an active session", async () => {
      const course = makeCourse({ userId: userAuthUser.id });
      const activeSession = makeStudySession({ userId: userAuthUser.id, status: StudySessionStatus.IN_PROGRESS });

      ownershipMock.resolveOwnerId.mockResolvedValue(userAuthUser.id);
      courseRepositoryMock.findOwnedById.mockResolvedValue(course);
      studySessionRepositoryMock.findActiveByUser.mockResolvedValue([activeSession]);
      ...
      await expect(promise).rejects.toBeInstanceOf(ValidationError);
      await expect(promise).rejects.toThrow("Usuário já possui uma sessão em andamento");
      expect(studySessionRepositoryMock.create).not.toHaveBeenCalled();
    });
  ...
  describe("update", () => {
    ...
    it("should update studiedAt successfully as the owner", async () => {
      const session = makeStudySession({ userId: userAuthUser.id, status: StudySessionStatus.COMPLETED });
      ...
      const result = await service.update(userAuthUser, session.id, { studiedAt: "01/03/2026" });
    ...
    it("should keep existing studiedAt when not provided", async () => {
      ...
      await service.update(userAuthUser, session.id, {});
    ...
    it("should allow ADMIN to update another user's session", async () => {
      ...
      await service.update(adminAuthUser, session.id, { studiedAt: "01/03/2026" });
    ...
    it("should throw ValidationError when session is IN_PROGRESS", async () => {
      const session = makeStudySession({ userId: userAuthUser.id, status: StudySessionStatus.IN_PROGRESS });

      studySessionRepositoryMock.findByIdWithOwner.mockResolvedValue(session);

      const promise = service.update(userAuthUser, session.id, { studiedAt: "01/03/2026" });

      await expect(promise).rejects.toBeInstanceOf(ValidationError);
      await expect(promise).rejects.toThrow("Só é possível editar sessões finalizadas");
      expect(studySessionRepositoryMock.update).not.toHaveBeenCalled();
    });
    ...
    it("should throw NotFoundError when session does not exist as USER", async () => {
      ...
      const promise = service.update(userAuthUser, "fake-id", { studiedAt: "01/03/2026" });
    ...
    it("should throw NotFoundError when session does not exist as ADMIN", async () => {
      ...
      const promise = service.update(adminAuthUser, "fake-id", { studiedAt: "01/03/2026" });
    ...
    it("should propagate findByIdWithOwner error for USER", async () => {
      ...
      const promise = service.update(userAuthUser, "any-id", { studiedAt: "01/03/2026" });
